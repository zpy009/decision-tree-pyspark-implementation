...
if __name__ == "__main__":
    ...
    import pickle as pkl
    X = data1.reset_index().drop(columns=["index"]).drop(columns=["label"]) 
    y= data1["label"]
    L=[]
    for i in range(8):
        num=int(50000*(i+1)/8)
        temp_X=X.loc[:num,:]
        temp_y=y.loc[:num]
        tree = DecisionTreeID3(max_depth=4,min_gain=-1,min_samples_split=-1)
        start = time.time()
        tree.fit(temp_X, temp_y)
        end=time.time()
        t=end-start
        L.append(t)
        print("Data shape: {} | ".format(temp_X.shape).ljust(20) + "  Time: {}".format(t))
